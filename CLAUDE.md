...
**Shared parent domain:** Decided out-of-band as part of NC-25 rollout — NOT in `.env.example`. `MAILBOX_SHARED_DOMAIN` is the env var the provisioning script reads; the operator sets it to the chosen domain (e.g. `staqsmail.com` — placeholder; final value pending registration). The Caddyfile itself only sees `{$DOMAIN}` which is the fully-resolved hostname.
...
# →  acme.mailbox.staqsmail.com  A  192.168.50.11   (proxied=false, TTL 60s)
```
...
**Caddy reads `DOMAIN` from the appliance's `.env`** — set it to the fully-resolved hostname (`acme.mailbox.staqsmail.com`) on the appliance. The Caddyfile's `tls { dns cloudflare {env.CLOUDFLARE_API_TOKEN} }` block is the same for shared-subdomain and customer-owned-domain appliances; only `DOMAIN` and the token's zone scope change between them.
