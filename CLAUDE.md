...
| Next.js 14 dashboard | App Router + Kysely | Approval queue UI + internal API routes | **DR-24**: dedicated Next.js service (`mailbox-dashboard`), not an Express+Vite SPA and not a Brain plugin. Internal routes: `/api/internal/{draft-prompt,draft-finalize,classification-prompt,classification-normalize,onboarding/live-gate,inbox-messages}` plus CRUD under `/api/drafts/...`. **Dashboard ORM ADR (2026-05-01)**: Kysely chosen over Prisma + Drizzle. Prisma rejected on Jetson hardware grounds (Rust query-engine sidecar + ARM64 memory + Prisma Date emit fighting the pg type-parser overrides); Drizzle rejected as redundant churn (its schema-DSL value is already covered by hand-authored `.sql` migrations + `kysely-codegen` introspection). Full reasoning in `.planning/STATE.md`. |
...
| 2026-05-01 ADR (Dashboard ORM) | Kysely chosen over Prisma + Drizzle. **Prisma**: rejected on Jetson hardware grounds (Rust query-engine sidecar process ~80-150MB ARM64 + migration-tooling fight with the 8 existing hand-authored `.sql` migrations + Prisma's `Date` emit contradicting the pg type-parser string overrides). **Drizzle**: rejected as redundant churn — its schema-DSL is its main value, but the existing hand-authored `.sql` migrations are an asset that `kysely-codegen` already introspects into typed row shapes. Closes STAQPRO-136. Full ADR in `.planning/STATE.md`. | Active |
