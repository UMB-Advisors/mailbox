...
- **Backfill from local rows — one-shot.** `npm run rag:backfill` (or the `mailbox-dashboard` container `npx tsx scripts/rag-backfill.ts`) backfills from `mailbox.inbox_messages` + `mailbox.sent_history` over a configurable lookback window (`RAG_BACKFILL_LOOKBACK_DAYS`, default 90). Idempotent on point UUID.
- **Backfill from Gmail history — one-shot, STAQPRO-194.** Open n8n editor → load `MailBOX-RAG-Backfill` workflow → click "Execute Workflow". Pulls last 6 months (`newer_than:180d -in:spam -in:trash`) via the existing Gmail OAuth credential, POSTs to `/api/internal/rag-backfill-batch` (cap 5000 rows; narrow the window for higher-volume accounts). Direction inferred server-side (operator email = `dustin@heronlabsinc.com` hardcoded in the workflow's "Set Operator Email" node — edit per-customer). Classification is set to `'unknown'` for backfilled rows (live forward-moving inbound continues through the live classify pipeline). Idempotent on Gmail message_id via deterministic point UUIDs — safe to re-run. Confirm via `/status` Qdrant card showing populated row count.
