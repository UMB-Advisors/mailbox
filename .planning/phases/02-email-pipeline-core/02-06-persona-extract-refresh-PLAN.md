...
- HIGH (data source): Persona extraction now reads from BOTH `mailbox.historical_sent` (onboarding seed corpus — 02-05 review fix) AND `mailbox.sent_history` (live approved sends — once any exist). The previous "fallback to Qdrant" comment was inaccurate; the code now spells out both sources and labels exemplar provenance with `source='historical' | 'live'` so PERS-05 can tell drift signal from ingest-time signal.
- HIGH (handoff contract): `09-persona-extract-trigger` now explicitly chains to `12-tuning-sample-generate` (02-08) after a successful extract, ending the disagreement between 02-06 and 02-08 about who launches tuning-sample generation. Onboarding flow → 06 → 09 → 12 → operator-rates → live.
- MEDIUM (timezone): Monthly refresh cron is now interpreted in the appliance's local time. The n8n container is configured with `TZ=${APPLIANCE_TZ:-America/Los_Angeles}` (also added to docker-compose.yml), and the cron expression `0 2 1 * *` therefore fires at 02:00 local time. The must-have and implementation now agree.
</review_fixes>

<objective>
Extract the operator's voice profile from ingested sent history using the hybrid approach locked in CONTEXT.md D-07 through D-11: deterministic statistical markers (sentence length distribution, formality score, greeting/closing frequencies) plus 3–5 per-category few-shot exemplars drawn from `mailbox.historical_sent` (onboarding seed) and `mailbox.sent_history` (live approved sends, once any exist). The extraction runs once at onboarding (chain: 06-rag-ingest-sent-history → 09-persona-extract-trigger → 12-tuning-sample-generate) and re-runs monthly via an n8n scheduled sub-workflow at 02:00 **local** time (review fix: n8n container TZ pinned to appliance TZ). Persona is stored in `mailbox.persona` as a single row with JSONB columns. The drafting path (Plan 02-07) reads the row to compose prompts.
</objective>
...
- `POST /api/persona/extract` endpoint runs the extraction job. **Data source priority (review fix):** uses `mailbox.sent_history` rows first (live approved sends carry real classification labels); if fewer than `N` rows are available, top up from `mailbox.historical_sent` (onboarding corpus). Heuristic keyword fallback only when classification_log is not joined. Idempotent.
- `GET /api/persona` returns the current persona row shape (for dashboard display and Plan 02-08 tuning session)
- Categories with fewer than 3 exemplars in the combined corpus record the gap explicitly in the persona row (D-09)
- Monthly refresh sub-workflow `08-persona-monthly-refresh` fires on the 1st of each month at **02:00 local time** (review fix: container TZ pinned; cron `0 2 1 * *` evaluated in local TZ), reads `mailbox.sent_history` for the last 30 days of approved sends, and updates the row (PERS-05)
- `09-persona-extract-trigger` is callable from the onboarding flow after the 6-month ingest completes, and from `POST /api/persona/extract`. **On successful extract (review fix), it dispatches `12-tuning-sample-generate` via the n8n REST API to start the tuning-sample run; failure to dispatch leaves a `tuning_dispatch_pending` flag on the onboarding row for the dashboard to surface.**
</must_haves>
...
export async function pickExemplars(targetPerCategory = 5): Promise<{ exemplars: ExemplarMap; gaps: ExemplarGaps[] }> {
  // Review fix: explicit two-source strategy.
  //   1. mailbox.sent_history (LIVE approved sends with real classification_log joins).
  //   2. mailbox.historical_sent (ONBOARDING corpus from 02-05's 6-month backfill).
  // Heuristic keyword matching is only used when no classification label exists
  // (typical for historical_sent rows ingested before the classifier ever ran).
  const live = (await db.execute(sql`
    ...
    LIMIT 500;
  `)).rows as Array<{ reply: string; subject: string; sent_at: any; inbound_body: string | null; category: string | null; source: 'live' }>;

  const historical = (await db.execute(sql`
    SELECT body_text AS reply,
           subject AS subject,
           sent_at AS sent_at,
           NULL::text AS inbound_body,
           NULL::text AS category,
           'historical'::text AS source
    FROM mailbox.historical_sent
    ORDER BY sent_at DESC NULLS LAST
    LIMIT 1500;
  `)).rows as Array<{ reply: string; subject: string; sent_at: any; inbound_body: string | null; category: string | null; source: 'historical' }>;

  // Live rows go first — real classification labels are more trustworthy
  // signal for category exemplars than keyword heuristics on historical rows.
  const joined = [...live, ...historical];
  ...
  const pushExemplar = (cat: string, row: any) => {
    ...
    exemplars[cat].push({
      ...
      source: row.source,                      // 'live' | 'historical' — review fix: provenance
    });
  };
  ...
  for (const row of joined) {
    if (row.category && CATEGORIES.includes(row.category)) {
      ...
      // Strategy B: keyword heuristic on reply + inbound (historical-only path)
...
export interface PersonaBuildResult {
  ...
  source_mix: { live: number; historical: number }; // review fix: provenance breakdown
...
export async function buildPersona(customerKey = 'default'): Promise<PersonaBuildResult> {
  // 1. Pull sent bodies. Review fix: read from BOTH sources explicitly.
  //    `sent_history` carries LIVE approved/generated outbound, including the
  //    classification provenance needed by PERS-05 drift tracking.
  //    `historical_sent` carries the onboarding 6-month backfill (02-05 fix)
  //    and is used to seed the persona while sent_history is empty.
  const liveBodies = (await db.execute(sql`
    SELECT COALESCE(draft_sent, '') AS body_text
    FROM mailbox.sent_history
    ORDER BY sent_at DESC LIMIT 2000;
  `)).rows.map((r: any) => String(r.body_text || '')).filter(Boolean);

  const histBodies = (await db.execute(sql`
    SELECT COALESCE(body_text, '') AS body_text
    FROM mailbox.historical_sent
    ORDER BY sent_at DESC NULLS LAST LIMIT 2000;
  `)).rows.map((r: any) => String(r.body_text || '')).filter(Boolean);

  // Use live + historical for marker stats; historical alone is fine before
  // the first live send.
  const bodies = [...liveBodies, ...histBodies];
  const sourceMix = { live: liveBodies.length, historical: histBodies.length };
  ...
  return {
    source_email_count: bodies.length,
...
<task id="5">
<action>
Create `n8n/workflows/09-persona-extract-trigger.json` — sub-workflow that invokes the dashboard backend's `POST /api/persona/extract` AND chains to `12-tuning-sample-generate` (review fix: ends the 02-06 ↔ 02-08 disagreement about who launches tuning-sample generation). Triggered by the onboarding flow (Plan 02-08) after the 6-month sent ingest completes. Node graph:

1. **Execute Workflow Trigger** — accepts `{ customer_key }` (defaults to `'default'`).
2. **HTTP Request: Extract** — `POST http://dashboard:3000/api/persona/extract`, expect 200 with JSON body `{ source_email_count, source_mix, markers, category_counts, gaps, updated_at }`.
3. **IF gaps is non-empty** — write a warning to stderr (`console.warn`) and continue.
4. **Postgres: Update onboarding** — `UPDATE mailbox.onboarding SET tuning_sample_count = 0, tuning_rated_count = 0, persona_built_at = NOW() WHERE customer_key = $1;` (resets counters before tuning-sample generation kicks off).
5. **HTTP Request: Dispatch tuning generator (review fix)** — looks up `12-tuning-sample-generate`'s workflow id via `GET http://n8n:5678/rest/workflows?filter=name:12-tuning-sample-generate` (credential `N8N Internal API`), then `POST http://n8n:5678/rest/workflows/<id>/run` with `{ customer_key }`. This is the SAME dispatch pattern used by 02-03/02-08 — no `run-by-name` invented endpoint.
6. **IF dispatch failed** — `UPDATE mailbox.onboarding SET tuning_dispatch_pending = true WHERE customer_key = $1;` so the dashboard can surface a retry. Otherwise reset the flag.
...
- `grep '12-tuning-sample-generate' n8n/workflows/09-persona-extract-trigger.json` matches (review fix: handoff chain)
- `grep '/rest/workflows/' n8n/workflows/09-persona-extract-trigger.json` matches (review fix: standard n8n dispatch pattern)
- `grep 'tuning_dispatch_pending' n8n/workflows/09-persona-extract-trigger.json` matches (review fix: surface failures)
</acceptance_criteria>
</task>

<task id="6">
<action>
Create `n8n/workflows/08-persona-monthly-refresh.json` — cron-triggered monthly refresh (D-10, PERS-05). Fires on the 1st of each month at 02:00 **local time** (review fix: n8n container TZ is pinned to the appliance TZ in `docker-compose.yml`, so `0 2 1 * *` is evaluated in local TZ, not UTC). Calls the same `POST /api/persona/extract` endpoint and logs the result to n8n execution history:

1. **Cron trigger** — `0 2 1 * *` interpreted in `$TZ`. The default `TZ` is `${APPLIANCE_TZ:-America/Los_Angeles}` set on the n8n service in `docker-compose.yml` (also added by this plan).
2. **HTTP Request** — `POST http://dashboard:3000/api/persona/extract`.
3. **Function: Log summary** — writes `source_email_count`, `source_mix`, `gaps`, and `updated_at` to `console.log` for audit via n8n execution log.
...
**docker-compose.yml update (review fix — TZ)**: add to the `n8n` service environment block (single-line so it diffs cleanly):

```yaml
  n8n:
    environment:
      TZ: ${APPLIANCE_TZ:-America/Los_Angeles}
      GENERIC_TIMEZONE: ${APPLIANCE_TZ:-America/Los_Angeles}
```

Both `TZ` (for the OS cron-style scheduling layer) and `GENERIC_TIMEZONE` (for n8n's own scheduler nodes) MUST agree, otherwise the cron fires at a different time than its expression suggests. `.env.example` should document `APPLIANCE_TZ`.
</action>
<read_first>
  - n8n/workflows/09-persona-extract-trigger.json
...
- Review fix: `docker compose exec -T n8n printenv TZ` returns the value of `APPLIANCE_TZ` (default `America/Los_Angeles`).
- Review fix: `docker compose exec -T n8n printenv GENERIC_TIMEZONE` matches `TZ` (both must agree, otherwise the cron fires off-time).
- Review fix: `grep -A 5 'n8n:' docker-compose.yml | grep -c 'TZ:'` returns at least `1`.
</acceptance_criteria>
</task>
