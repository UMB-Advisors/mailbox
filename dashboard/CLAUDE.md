...
- `app/api/classifications/reclassify-sender/route.ts` — `POST` operator reclassify-by-**sender** (MBOX-368, extends MBOX-123). Body `{ email, category, reason? }` (zod `reclassifyBySenderBodySchema`; `email` normalized via `extractAddress`, `category` anchored to `CATEGORIES`). Keyed on the sender address, not a draft id, so it reaches `spam_marketing` rows that were dropped (no draft). One txn (`lib/queries-sender-overrides.ts:reclassifyBySender`): upserts a `mailbox.sender_classification_overrides` row (the **sticky rule** for future inbound), relabels every `inbox_messages` from that sender (bare-address SQL match mirroring `extractAddress`), appends one `classification_log` audit row per message (`model_version='operator-sender-override'`), and relabels any existing drafts. **Relabel only** — no drafts generated for historical dropped mail (operator decision 2026-05-29). The FUTURE half is enforced at classify time in `classification-normalize` (see below) — no n8n change.
...
- **MBOX-368 sender-override (highest-precedence preclass)**: before the sync normalize chain AND the async owns-thread guard, the route consults `mailbox.sender_classification_overrides` (`lib/classification/sender-override.ts:getSenderOverride`, keyed on `extractAddress(from)`). A hit forces that category (`confidence=1`, `preclass_source='sender-override'`, `route=routeFor(category,1)`) and returns immediately — an explicit operator reclassify (via `POST /api/classifications/reclassify-sender`) is authoritative over the LLM and every heuristic. The model's raw JSON is preserved in `raw_output` for forensics. Lookup is fail-open (DB error → normal classify path) and kill-switchable via `SENDER_OVERRIDE_PRECLASS_DISABLE=1`. This is the FUTURE half of reclassify-by-sender — it needs no n8n change because the `Normalize` node already sends `from`.
