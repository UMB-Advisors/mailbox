...
|-----|-------------|-------|
| **FR-MP-1** | Threading normalization | Canonical `thread_id` for every provider. Gmail → thread id; Graph → `conversationId`; IMAP → deterministic hash of the `References` chain root (fall back to `In-Reply-To`, then subject-normalization as last resort). `thread-history.ts` keys on this unchanged. |
| **FR-MP-2** | Per-provider quote/signature stripping | `strip-quoting.ts` is tuned to Gmail. Outlook uses `From: … Sent: …` blocks; IMAP clients vary wildly. Strategy object per provider; default heuristic + provider override. |
...
| **S-MP-4 (DR-56)** | Is dashboard-owned provider I/O worth retiring n8n ingress? | A `setInterval` poll + one provider adapter matches n8n reliability over a 48h soak | n8n parity not reached in a sprint → keep n8n at the edge (Option A), per-provider workflows |
...
> **DR-56 (Candidate, gated on S-MP-4) — n8n stays orchestrator; provider I/O moves into the dashboard.**
> Multi-provider is the forcing function to execute STAQPRO-187's narrow scope: ingress poll + egress send become dashboard-owned per-provider adapters; n8n keeps classify/draft orchestration (or a thin schedule trigger). **Alternative kept (Option A):** per-provider n8n workflows (`MailBOX-Imap`, `MailBOX-Graph`) selected by `accounts.provider`, normalization done in dashboard internal routes. Decided by spike, not by fiat — see §7.
